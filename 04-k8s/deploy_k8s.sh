...
yandex_cloud_controller_manager:
  folder_id: "$(terraform -chdir=../01-sa_bucket output -raw folder_id)"
...
access_key=$(terraform -chdir=../01-sa_bucket output -raw sa_access_key)
secret_key=$(terraform -chdir=../01-sa_bucket output -raw sa_secret_key)
cat << EOF > sa-key.json
{
...
EOF
for ip in $(terraform -chdir=../02-infra output -json worker_ips | jq -r '.[]' && echo $control_plane_ip); do
  ssh -i ../02-infra/id_ed25519 ubuntu@$ip "sudo mkdir -p /etc/yandex-cloud && sudo chown ubuntu:ubuntu /etc/yandex-cloud"
  scp -i ../02-infra/id_ed25519 sa-key.json ubuntu@$ip:/etc/yandex-cloud/sa-key.json
done
rm sa-key.json
...
ssh -i ../02-infra/id_ed25519 ubuntu@$control_plane_ip "mkdir -p ~/.kube && sudo cp /etc/kubernetes/admin.conf ~/.kube/config && sudo chown ubuntu:ubuntu ~/.kube/config"

scp -i ../02-infra/id_ed25519 ubuntu@$control_plane_ip:/home/ubuntu/.kube/config ~/.kube/config
